#!/usr/bin/env python3
"""
Self-Supervised Denoising Training Script
"""
...
def parse_args():
    ...
    # Model arguments
    parser.add_argument("--noisetype", type=str, default="poisson30",
    ...
    parser.add_argument("--n_snapshot", type=int, default=10, help="Save model every n epochs")
...
def setup_training(opt):
    ...
    # Set up logger
    utils.setup_logger("train", opt.save_path, "train_" + opt.log_name,
...
def create_data_loaders(opt):
    ...
    # Training dataset
    training_dataset = utils.ImageDataset(opt.data_dir, patch=opt.patchsize)
    ...
    valid_dict = {"data_THG": utils.load_validation_data(data_dir)}
    
    return training_loader, valid_dict


def create_optimizer_scheduler(network, opt):
...
def get_loss_weights(epoch, opt):
    ...
    if Lambda <= Thread1:
        ...
        beta = opt.Lambda2 + (Lambda - Thread1) * (opt.increase_ratio - opt.Lambda2) / (Thread2 - Thread1)
    else:
        beta = opt.increase_ratio
    
    alpha = opt.Lambda1
    return alpha, beta


def train_epoch(network, training_loader, optimizer, masker, opt, epoch, logger):
    """Train for one epoch"""
    network.train()
    ...
    alpha, beta = get_loss_weights(epoch, opt)
    ...
    for iteration, clean in enumerate(training_loader):
        ...
        logger.info(
            "{:04d} {:05d} diff={:.6f}, exp_diff={:.6f}, Loss_Reg={:.6f}, Lambda={:.3f}, Loss_Rev={:.6f}, Loss_All={:.6f}, Time={:.4f}".format(
                epoch, iteration, torch.mean(diff**2).item(), torch.mean(exp_diff**2).item(),
                loss_reg.item(), epoch / opt.n_epoch, loss_rev.item(), loss_all.item(), time.time() - st
            )
        )


def validate_model(network, valid_dict, masker, opt, epoch, systime, logger):
    """Validate the model"""
    network.eval()
    ...
    np.random.seed(101)
    valid_repeat_times = {"data_THG": 3}
    _, beta = get_loss_weights(epoch, opt)
    ...
    for valid_name, valid_images in valid_dict.items():
        ...
        for i in range(repeat_times):
            for idx, im in enumerate(valid_images):
                ...
                # Process outputs
                pred_dn = dn_output[:, :, :H, :W].permute(0, 2, 3, 1).cpu().data.clamp(0, 1).numpy().squeeze(0)
                pred_exp = exp_output.detach().clone()[:, :, :H, :W].permute(0, 2, 3, 1).cpu().data.clamp(0, 1).numpy().squeeze(0)
                pred_mid = (dn_output[:, :, :H, :W] + beta * exp_output.detach().clone()[:, :, :H, :W]) / (1 + beta)
                ...
                if i == 0:
                    if epoch == opt.n_snapshot:
                        Image.fromarray(origin255).convert("RGB").save(
                            os.path.join(save_dir, "{}_{:03d}-{:03d}_clean.png".format(valid_name, idx, epoch))
                        )
                        Image.fromarray(noisy255).convert("RGB").save(
                            os.path.join(save_dir, "{}_{:03d}-{:03d}_noisy.png".format(valid_name, idx, epoch))
                        )
        ...
        with open(log_path, "a") as f:
            f.write("epoch:{},dn:{:.6f}/{:.6f},exp:{:.6f}/{:.6f},mid:{:.6f}/{:.6f}\n".format(
...
def main():
    ...
    # Setup training environment
    systime, logger = setup_training(opt)
    
    # Create model
    network = create_model(opt)
    
    # Create data loaders
    training_loader, valid_dict = create_data_loaders(opt)
    
    # Create optimizer and scheduler
    optimizer, scheduler = create_optimizer_scheduler(network, opt)
    ...
    if opt.resume is not None:
        epoch_init, optimizer, scheduler = utils.resume_state(opt.resume, optimizer, scheduler)
    
    # Load pretrained model if specified
    if opt.checkpoint is not None:
        network = utils.load_network(opt.checkpoint, network, strict=True, logger=logger)
    ...
    print("Batchsize={}, number of epoch={}".format(4, opt.n_epoch))
    
    # Training loop
    for epoch in range(epoch_init, opt.n_epoch + 1):
        # Train one epoch
        train_epoch(network, training_loader, optimizer, masker, opt, epoch, logger)
        ...
        if epoch % opt.n_snapshot == 0 or epoch == opt.n_epoch:
            # Save model
            utils.save_network(network, opt.save_path, epoch, "model", logger)
            
            # Validate model
            validate_model(network, valid_dict, masker, opt, epoch, systime, logger)
