...
class BiLevelRoutingAttention(nn.Module):
    ...
    def __init__(
        ...
        if self.kv_downsample_mode == "ada_avgpool":
            ...
            self.kv_down = (
                nn.MaxPool2d(self.kv_downsample_ratio) if self.kv_downsample_ratio > 1 else nn.Identity()
            )
        elif self.kv_downsample_mode == "avgpool":
            assert self.kv_downsample_ratio is not None
            self.kv_down = (
                nn.AvgPool2d(self.kv_downsample_ratio) if self.kv_downsample_ratio > 1 else nn.Identity()
            )
    ...
    def forward(self, x, ret_attn_mask=False):
        ...
        if self.auto_pad:
            ...
            x = F.pad(x, (0, 0, pad_l, pad_r, pad_t, pad_b))  # dim=-1  # dim=-2  # dim=-3
...
class biTransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        n_win_h = max(1, H // window_size)
        n_win_w = max(1, W // window_size)
        assert n_win_h == n_win_w, "window_size must be square"
        n_win = n_win_h
        auto_pad = (H % window_size != 0) or (W % window_size != 0)
...
class UpSample(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.factor == 2:
            out = out.view(B, -1, C // 2)  # (B, H*W, C//2)
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Could not calculate FLOPs:", e)
