#!/usr/bin/env python3
"""
Self-Supervised Denoising Testing Script
"""
...
def parse_args():
    ...
    parser.add_argument("--beta", type=float, default=20.0,
                        help="Beta parameter for combining outputs")
...
def setup_testing(opt):
    ...
    # Set up logger
    utils.setup_logger("test", opt.save_test_path, "test_" + opt.log_name,
...
def create_model(opt, logger):
    ...
    network = utils.load_network(opt.checkpoint, network, strict=False, logger=logger)
    network.eval()
...
def load_test_data(opt):
    """Load test dataset"""
    data_dir = os.path.join(opt.test_dirs, "data_THG")
    test_dict = {"data_THG": utils.load_validation_data(data_dir)}
    return test_dict
...
def process_image(image, network, masker, beta):
    ...
    noisy_im = im  # In this case, we're not adding noise for testing
    ...
    with torch.no_grad():
        ...
        # Get denoised output using masking
    ...
    return {
        ...
        "metrics": {
            "psnr_dn": psnr_dn, "ssim_dn": ssim_dn,
            "psnr_exp": psnr_exp, "ssim_exp": ssim_exp,
            "psnr_mid": psnr_mid, "ssim_mid": ssim_mid
        }
    }


def test_dataset(network, test_dict, masker, opt, logger):
    ...
    for valid_name, valid_images in test_dict.items():
        ...
        for i in range(repeat_times):
            for idx, image in enumerate(valid_images):
                # Process image
                results = process_image(image, network, masker, opt.beta)
                ...
                logger.info(
                    ...
                    "PSNR_EXP: {:.6f} dB; SSIM_EXP: {:.6f}; PSNR_MID: {:.6f} dB; SSIM_MID: {:.6f}".format(
                        valid_name, i, idx, 
                        results["metrics"]["psnr_dn"], results["metrics"]["ssim_dn"],
                        results["metrics"]["psnr_exp"], results["metrics"]["ssim_exp"],
                        results["metrics"]["psnr_mid"], results["metrics"]["ssim_mid"]
                    )
                )
        ...
        logger.info(
            ...
            "\tPSNR_MID: {:.6f} dB; SSIM_MID: {:.6f}".format(
                valid_name,
                avg_metrics["psnr_dn"], avg_metrics["ssim_dn"],
                avg_metrics["psnr_exp"], avg_metrics["ssim_exp"],
                avg_metrics["psnr_mid"], avg_metrics["ssim_mid"]
            )
        )
...
def main():
    ...
    # Setup testing environment
    logger = setup_testing(opt)
    
    # Create and load model
    network = create_model(opt, logger)
    
    # Load test data
    test_dict = load_test_data(opt)
    ...
    # Test the model
    avg_metrics = test_dataset(network, test_dict, masker, opt, logger)
